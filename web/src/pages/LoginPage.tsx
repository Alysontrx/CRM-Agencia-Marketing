...
import { ArrowRight, Sparkles, User, Lock, Mail } from 'lucide-react';
  ...
  const loginWithGoogle = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (userInfo.email) {
          let ok = await login(userInfo.email);
          if (!ok) {
            // User doesn't exist, auto-register since they passed Google Auth (which is restricted to Test Users anyway)
            // Fetch max ID to avoid primary key conflicts
            const { data: maxUser } = await supabase.from('usuarios').select('id').order('id', { ascending: false }).limit(1).single();
          ...
          if (ok) {
            ...
            setError('Falha inesperada ao logar após registro.');
          }
        }
      } catch (err) {
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-4 py-3">
              ...
              <span className="text-xs text-zinc-500 font-medium uppercase tracking-wider">ou continue com</span>
              <div className="h-px bg-zinc-800 flex-1" />
            </div>
            ...
            <button 
              ...
              className="w-full h-12 bg-zinc-900 border border-zinc-800 text-zinc-300 font-medium rounded-xl hover:bg-zinc-800 transition-all flex items-center justify-center gap-3 relative overflow-hidden disabled:opacity-50" 
              disabled={loading}
            >
