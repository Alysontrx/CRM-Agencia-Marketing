// Atlas OS — Agência: Frontend Engine & Operational Controller
...
const STATUS_BADGE_MAP = {
    ...
    'Aguardando revisão': 'badge aguardando_revisao',
    'Em correção': 'badge em_correcao',
...
const CLIENT_STATUS_LABELS = {
    'em_dia': '🟢 Em Dia',
    'atencao': '🟡 Atenção / Aguardando Material',
...
function handleManualLogin() {
    ...
    if (user) {
        ...
        alert('Credenciais incorretas. Use um dos atalhos rápidos ou tente novamente.');
    }
}
...
function handleShortcutLogin(profileName) {
    ...
    if (user) {
        ...
        const fallback = {
            ...
            Marina: { id: 4, nome: 'Marina', funcao: 'Secretária/Revisora', avatar: 'https://images.unsplash.com/photo-1567532939604-b6b5b0db2604?w=150&auto=format&fit=crop&q=80' }
        }[profileName];
        loginAs(fallback);
    }
}
...
function switchView(viewId) {
    ...
    const formattedTitle = {
        ...
        'atlas-ai': 'Atlas Inteligência Artificial',
...
function renderDashboard() {
    ...
    STATE.tasks.forEach(t => {
        ...
        if (isTaskOverdue) {
            ...
            const responsible = (STATE.users.find(u => u.id === t.responsavel_id) || {nome: 'Sem alocação'}).nome;
            attentionContainer.innerHTML += `
                <div class="attention-item">
    ...
    if (alertsCount === 0) {
        attentionContainer.innerHTML = `
            <div style="text-align: center; padding: 20px; color: var(--text-muted);">
                ...
                <p>Nenhum alerta crítico pendente. A operação está rodando perfeitamente!</p>
            </div>
        `;
    }
    ...
    if (todayDeliveries.length > 0) {
        todayDeliveries.forEach(t => {
            const clientName = (STATE.clients.find(c => c.id === t.cliente_id) || {nome: 'Geral'}).nome;
            const assigneeName = (STATE.users.find(u => u.id === t.responsavel_id) || {nome: 'Não alocado'}).nome;
            deliveriesContainer.innerHTML += `
                <tr>
...
function renderClients() {
    ...
    STATE.clients.forEach(c => {
        const responsible = (STATE.users.find(u => u.id === c.responsavel_id) || {nome: 'Sem responsável'}).nome;
...
function openClientDetail(clientId) {
    ...
    // Pipeline tabs: reset to overview and render materials/pendings
    if (typeof switchClientTab === 'function') {
        switchClientTab('tab-overview', document.querySelector('.client-tab-btn'));
    }
    if (typeof renderClientMaterials === 'function') renderClientMaterials(clientId);
    if (typeof renderClientPendings === 'function') renderClientPendings(clientId);
}
...
function renderClientTimeline(client) {
    ...
    const milestones = [
        ...
        { label: 'Design e Produção das Mídias', minProgress: 60 },
        { label: 'Revisão Ortográfica e Coesão', minProgress: 75 },
        { label: 'Aprovação Final da Administradora', minProgress: 85 },
        { label: 'Publicações Agendadas', minProgress: 95 },
        { label: 'Relatório Mensal Gerado', minProgress: 100 }
    ];
...
function renderClientMetricsSimulator(client) {
    ...
    container.innerHTML = `
        <div class="mini-card">
            <span style="font-size:12px; color:var(--text-secondary);">Cliques no site este mês</span>
...
function renderTasks() {
    ...
    if (STATE.currentUser && STATE.currentUser.funcao !== 'Administradora') {
        ...
        filteredTasks = filteredTasks.filter(t => {
            return t.responsavel_id === STATE.currentUser.id || 
                   ...
                   (userFuncao === 'Secretária/Revisora' && t.setor === 'Secretária');
        });
    }
    ...
    filteredTasks.forEach(t => {
        ...
        const statuses = ['A fazer', 'Em andamento', 'Aguardando material', 'Aguardando revisão', 'Em correção', 'Aprovado', 'Publicado', 'Finalizado', 'Atrasado'];
...
async function openTaskComments(taskId) {
    ...
    const newComment = prompt(`Anotações/Comentários para a tarefa: "${task.titulo}"${commentList}\n\nEscreva sua nova anotação abaixo:`);
    if (newComment) {
...
function renderCorrections() {
    ...
    STATE.corrections.forEach(c => {
        ...
        const responsible = (STATE.users.find(u => u.id === c.responsavel_id) || {nome: 'Sem alocação'}).nome;
...
async function handleCorrectionStatusChange(correctionId, newStatus) {
    ...
    if (corr) {
        ...
        // If status changed to "Corrigido", update parent task to "Aguardando revisão" so secretary approves it
        if (newStatus === 'Corrigido') {
            const task = STATE.tasks.find(t => t.id === corr.tarefa_id);
            if (task) {
                task.status = 'Aguardando revisão';
                await DB.updateTask(task.id, { status: 'Aguardando revisão' });
            }
        }
...
async function addCorrectionComment(corrId) {
    ...
    const newComment = prompt(`Comentários da Correção:${commentList}\n\nEscreva sua nota abaixo:`);
    if (newComment) {
...
function loadReportData() {
    ...
    const report = STATE.reports.find(r => r.cliente_id === selectedClientId) || {
        ...
        cliques_whatsapp: 10,
        engajamento: 2.1,
        observacoes: 'Dados simulados para demonstração inicial.'
    };
...
function runAIDaySummary() {
    ...
    STATE.clients.forEach(c => {
        if (c.status_geral === 'atencao' || c.status_geral === 'atrasado') {
            alertsHtml += `• <strong>${c.nome}</strong>: ${c.pendencia_atual}\n`;
        }
    });
    ...
    if (correctionsCount > 0) {
        suggestionHtml += `3. Priorizar e revisar correções abertas (${correctionsCount} pendentes) antes de criar novas demandas.`;
    }

    const reportText = `<strong>Diagnóstico Operacional de Hoje:</strong>

Hoje existem <strong>${clientsCount} clientes ativos</strong> na agência.
⚠️ <strong>${attentionClients.length} clientes</strong> precisam de atenção operacional.
⏳ <strong>${awaitingMaterialCount} clientes</strong> estão aguardando envio de material.
🚨 <strong>${overdueCount} tarefas</strong> estão registradas como atrasadas.
✏️ <strong>${correctionsCount} correções</strong> estão pendentes de ajuste pela equipe.
✅ A equipe já publicou/concluiu <strong>${finishedToday} demandas</strong> nesta rodada.

<strong>Alertas Críticos Ativos:</strong>
${alertsHtml}
<strong>Sugestão de Prioridades Recomendadas pela IA:</strong>
${suggestionHtml || 'Parabéns! Sua agência está sem gargalos e com todas as prioridades em dia.'}`;
...
Resumo rápido da operação da agência:

🟢 ${activeClientsInDay} clientes em dia
🟡 ${awaitingMaterialCount} clientes aguardando material
🔴 ${overdueCount} tarefas atrasadas
✏️ ${correctionsCount} correções pendentes
...
function renderSmartSuggestions() {
        ...
        if (designerTasks > 4) {
            container.innerHTML += `
                <div class="suggestion-item">
                    ...
                    <span><strong>Alocação desequilibrada:</strong> A designer ${designer.nome} está com ${designerTasks} demandas pendentes. Recomenda-se remanejar ou espaçar as datas.</span>
                </div>
            `;
        }
    }
    ...
    STATE.clients.forEach(c => {
        if (c.status_geral === 'atencao' && c.pendencia_atual.includes('Aguardando aprovação')) {
            container.innerHTML += `
                <div class="suggestion-item">
                    <i class="fa-solid fa-lightbulb"></i>
                    <span><strong>Gargalo do Cliente:</strong> O projeto de <strong>${c.nome}</strong> está parado por aprovação. Recomenda-se enviar mensagem de cobrança.</span>
                </div>
            `;
        }
    });
    ...
    if (pendingCorr > 2) {
        container.innerHTML += `
            <div class="suggestion-item">
                <i class="fa-solid fa-lightbulb"></i>
                <span><strong>Foco em Ajustes:</strong> Existem ${pendingCorr} correções no funil. Reduza a abertura de novos briefings para limpar a fila.</span>
            </div>
        `;
    }
    ...
    if (container.innerHTML === '') {
        container.innerHTML = `
            <div style="font-size:12px; color:var(--text-muted); text-align:center; padding:10px;">Sem sugestões operacionais extras no momento. Tudo correndo bem!</div>
        `;
    }
}
...
O cliente está com ${client.progresso}% das entregas do mês concluídas (${doneTasks}/${totalTasks} demandas).
Existem ${correctionCount} correções pendentes nos posts.
A próxima entrega de material está prevista para ${nextDelivery}.
...
Estamos passando para lembrar que ainda precisamos dos materiais pendentes (${client.pendencia_atual.toLowerCase()}) para dar continuidade ao seu projeto. Assim que recebermos, conseguimos avançar para as próximas etapas de design e redação.

Ficamos à disposição!`;
    } else {
        text = `Olá, tudo bem?

Gostaria de informar que o status do seu projeto está em ${client.progresso}% de conclusão neste mês.

Já temos os relatórios e os posts agendados para a próxima semana. Se tiver alguma dúvida ou novo material para enviar, nos envie por aqui.
...
Tenha um excelente dia!`;
    ...
    alert('Mensagem copiada para a área de transferência! Cole diretamente no WhatsApp do cliente.');
}
